...
const SidebarItem: React.FC<SidebarItemProps> = ({ to, icon, text, isNew }) => {
  return (
    <NavLink
      to={to}
      className={({ isActive }) => 
    ...
    >
      ...
      {isNew && (
        <span className="ml-auto text-[10px] font-bold bg-primary-500 text-white px-1.5 py-0.5 rounded-full">NEW</span>
      )}
    </NavLink>
  );
};
...
export const Sidebar: React.FC = () => {
  ...
  const entrepreneurItems = [
    ...
    { to: '/calendar', icon: <Calendar size={20} />, text: 'Calendar', isNew: true },
    { to: '/video-call', icon: <Video size={20} />, text: 'Video Calls', isNew: true },
    { to: '/messages', icon: <MessageCircle size={20} />, text: 'Messages' },
    { to: '/notifications', icon: <Bell size={20} />, text: 'Notifications' },
    { to: '/document-chamber', icon: <FolderOpen size={20} />, text: 'Document Chamber', isNew: true },
    { to: '/documents', icon: <FileText size={20} />, text: 'Documents' },
    { to: '/payments', icon: <CreditCard size={20} />, text: 'Payments', isNew: true },
  ];
  ...
  const commonItems = [
    { to: '/security', icon: <Shield size={20} />, text: 'Security', isNew: true },
